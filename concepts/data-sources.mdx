---
title: "Data sources"
description: "The 13 data sources behind Hive, and what each one contributes."
---

Hive normalizes 13 data sources behind one interface. Ten are external providers. The other
...
That covers every source in the table above except RWA Perps, which is not a managed key. If a
source needs a credential Hive does not hold, calls to it return `missing_key` rather than
failing silently, so you can tell the difference from an outage.
