...
allowance call, taken when the request is accepted and given back if the call fails, bad
arguments included, so only calls that succeed count toward the 25. `GET /api/v1/quota` and
`hive doctor` show what is left without spending any.
...
provider is reachable, so a validation error costs nothing. If the call then fails, the credit
is refunded and the receipt carries a warning saying so, which means only a call that succeeds
costs a credit. A scheduled monitor run is refunded the same way when it errors or every tool
call in it fails. Each receipt says what happened in `credit_cost`, `credits_used` and
`credits_remaining` (see [receipts](/concepts/receipts#what-the-call-cost)).
