...
The credit error tells you to upgrade or wait for the next cycle, but it does not carry a reset
timestamp or a `Retry-After` header. Only the per-minute rate limit and the anonymous daily
quota tell you when to retry.
