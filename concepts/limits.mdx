...
Requests fail with a quota error that names the cause and the reset time. Nothing is billed
beyond your subscription, and no request silently degrades to worse data.

Your credits reset at the start of each billing cycle. You can see the current balance and
cycle end on [dashboard usage](https://www.hiveintelligence.xyz/dashboard/usage).
