...
<ResponseField name="credits_used" type="integer">
  What was actually debited for this call. On a keyed lane it is 0 when argument validation
  or a gate stopped the call before the provider was reached, and the `credit_cost` once the
  provider was reached, even if the provider then failed (there is no refund path). On the
  keyless lane one allowance call is consumed per material request, valid or not, so the
  field reads 0 on a validation failure while the allowance still moved by one.
</ResponseField>
