...
| `hive archive` | `coverage`, `funding`, `oi`, `basis`, `long-short`, `liquidations` |
...
| `hive doctor` | Check API key, endpoint reachability, tool catalog, updates, and today's keyless allowance |
| `hive upgrade` | Update the CLI |
| `hive uninstall` | Remove Hive MCP entries, delete copied skill packs, and remove any routing block this CLI wrote |
| `hive routing install` | Append the Hive routing block to this project's AGENTS.md, or CLAUDE.md when there is no AGENTS.md. `--file` picks one, `--create` makes it, `--dry-run` previews |
| `hive routing decline` | Record that this project does not want the routing block, so nothing asks again |
| `hive routing remove` | Remove the block and restore the file's original bytes |

`--all` installs all 18 skill packs into your clients' skills directories. Pass `--skip-skills`
to configure the clients without them, and `--skip-verify` to keep the whole command
non-interactive, which is what an agent should run. Both `init` and `uninstall` accept
`--dry-run` if you want to see what would change first.

`hive init` never reads or writes AGENTS.md or CLAUDE.md. The routing block is written only by
`hive routing install`, and `HIVE_ROUTING=never` disables that on a machine.
...
| `hive tools info <name>` | Show a tool's parameter table (name, type, required, enum values, default), response schema, and description |
| `hive tools call <name> --<param> <value>` | Execute a tool with generated flags typed from its schema, in kebab-case or snake_case |
| `hive tools call <name> --args '<json>'` | The same call with JSON instead. Flags win over `--args`, and JSON can be piped on stdin |
...
| `hive feedback "<message>"` | Send one line of product feedback, up to 1,000 characters. `--tool` names the tool and `--receipt` ties it to a receipt, defaulting to your last call's |
