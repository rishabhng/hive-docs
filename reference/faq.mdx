...
<AccordionGroup>
  ...
  <Accordion title="Do I need my own provider API keys?">
    Almost never. Hive holds the upstream credentials and manages their rate limits for every
    source except RWA Perps. If a source needs a credential Hive does not hold, the call
    returns `missing_key` so you can tell it apart from an outage.
  </Accordion>
