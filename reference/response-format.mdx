...
```json
{
  "_hive": {
    ...
    "credit_cost": 1,
    "credits_used": 1,
    "credits_remaining": 9774
  }
}
```

`_hive` also carries `receipt_version` (always `"1.0"`; the credit fields were added without
bumping it), `server_version`, `input_digest`, and `result_digest`. `credit_cost`,
`credits_used` and `credits_remaining` are described under
[receipts](/concepts/receipts#what-the-call-cost); on an error the object also carries
`cause`, `next_action` and `doc_url`. The digests are SHA-256 self-checks over the canonical arguments and the
...
```json
{
  ...
  "meta": {
    ...
    "credit_cost": 1,
    "credits_used": 1,
    "credits_remaining": 9775,
    "quota_remaining": 9775
  }
}
```

`quota_remaining` is a deprecated alias of `credits_remaining` kept for one release; read
the new name. `meta.tool` is the tool you called and `meta.provider` is the surface that served it, so a hero
...
Failures return the same four legs on every surface: `code`, `message`, `cause`,
`next_action`, plus `retryable`, `retry_after` when a wait is known, and `doc_url`, an anchor
on the [errors page](/reference/errors). REST keeps `type`, `param` and `request_id`
alongside; JSON-RPC cap errors carry the fields under `error.data`; the CLI keeps
`suggestion` as an alias of `next_action` for one release.
